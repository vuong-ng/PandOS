...
void main() {
	...
	*buffer = 42;  /*buffer[0] = 'a'; */
	dstatus = SYSCALL(DISK_PUT, (int)buffer, 0, 3);
	...
	*buffer = 100;  /*buffer[0] = 'z'; */
	dstatus = SYSCALL(DISK_PUT, (int)buffer, 0, 23);

	dstatus = SYSCALL(DISK_GET, (int)buffer, 0, 3);
	...
	dstatus = SYSCALL(DISK_GET, (int)buffer, 0, 23);
	...
	i = 0;
	dstatus = SYSCALL(DISK_GET, (int)buffer, 0, i);
	...
	/* try to do a disk read into protected RAM */
	SYSCALL(DISK_GET, SEG1, 0, 3);
